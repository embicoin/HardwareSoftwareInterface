...
void place(BlockInfo *ptr, size_t reqSize)
{
  size_t blk_size = SIZE(ptr->sizeAndTags);
  size_t left_size = blk_size - reqSize;
  size_t precedingBlockUseTag = ptr->sizeAndTags & TAG_PRECEDING_USED;
 
  BlockInfo *new_block;

  if (left_size < MIN_BLOCK_SIZE) {
    ptr->sizeAndTags = blk_size | TAG_USED | precedingBlockUseTag;
    *((size_t *)POINTER_ADD(ptr, blk_size - WORD_SIZE)) = ptr->sizeAndTags;
    // if and only if there is no one bit of space left in one piece of memory,
    // mark the heap footer as TAG_PRECEDING_USED,  for future requestMoreSpace
    // and coalesce to use
    // ATTENTION: don't forget '|', or the next block's sizeAndTags will be replaced by TAG_PRECEDING_USED
    *((size_t *)POINTER_ADD(ptr, blk_size)) |= TAG_PRECEDING_USED;
    return;
  }

  ptr->sizeAndTags = reqSize | TAG_USED | precedingBlockUseTag;
  // allocated block doesn't need block footer
  //*((size_t *)POINTER_ADD(ptr, reqSize - WORD_SIZE)) = ptr->sizeAndTags;

  new_block = (BlockInfo *)POINTER_ADD(ptr, reqSize);
  // ATTENTION: the TAG_PRECEDING_USED must be within the block header(may not be within the block footer),
  // to be used in coalesceFreeBlock's while loop to judge if preceding block is used
  new_block->sizeAndTags = left_size | TAG_PRECEDING_USED;
  //*((size_t *)POINTER_ADD(new_block, left_size - WORD_SIZE)) = left_size | TAG_PRECEDING_USED;
  *((size_t *)POINTER_ADD(new_block, left_size - WORD_SIZE)) = left_size;
...
void* mm_malloc (size_t size) {
  ...
  // Add one word for the initial size header, header(sizeAndTags)'s size is 4 bytes under 32bit OS,
  // so for alignment, we have to add one blank word(4 bytes under 32bit os) to the head
  ...
  if ((ptrFreeBlock = searchFreeList(reqSize)) == 0) {
  ...
  // ATTENTION: stride the header, return the pointer to the payload
  return (BlockInfo *)POINTER_ADD(ptrFreeBlock, WORD_SIZE);
}
...
void mm_free (void *ptr) {
  ...
  // ATTENTION: the pointer returned by mm_malloc is the one to the payload,
  // so we have to minus WORD_SIZE to get the one to the header
  BlockInfo *p = (BlockInfo *)POINTER_SUB(ptr, WORD_SIZE);
  payloadSize = SIZE(p->sizeAndTags);

  // Implement mm_free.  You can change or remove the declaraions
  // above.  They are included as minor hints.

  // header
  p->sizeAndTags &= ~TAG_USED;
  // footer
  *((size_t *)POINTER_ADD(p, payloadSize - WORD_SIZE)) = p->sizeAndTags;
  followingBlock = (BlockInfo *)POINTER_ADD(p, payloadSize);
...
static void examine_heap() {
  ...
  /*
  //for(block = (BlockInfo*)POINTER_ADD(mem_heap_lo(), WORD_SIZE);
  //    SIZE(block->sizeAndTags) != 0 && block < mem_heap_hi();
  //    block = (BlockInfo*)POINTER_ADD(block, SIZE(block->sizeAndTags)))
      */

  for(block = FREE_LIST_HEAD; /* first block on heap */
      block != 0 && SIZE(block->sizeAndTags) != 0 && block < mem_heap_hi();
      block = block->next) {

    /* print out common block attributes */
    fprintf(stderr, "%p: %ld %ld %ld %d %d\t",
            (void*)block,
            SIZE(block->sizeAndTags),
            block->sizeAndTags & TAG_PRECEDING_USED,
            block->sizeAndTags & TAG_USED,
            *((int *)POINTER_ADD(block, SIZE(block->sizeAndTags) - WORD_SIZE)),
            *((int *)POINTER_ADD(block, SIZE(block->sizeAndTags))));
...
int mm_check() {
  BlockInfo *p = FREE_LIST_HEAD;
  if (p == NULL)
    return 0;

  if ((p->sizeAndTags & TAG_PRECEDING_USED) == 0) {
    printf("tag error, %d\n", __LINE__);
    getchar();
    exit(1);
  }
